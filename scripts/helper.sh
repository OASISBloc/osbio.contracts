...
function eosio-directory-prompt() {
    ...
    while true; do
      if [[ $NONINTERACTIVE != true ]]; then
        if [[ -z $EOSIO_VERSION ]]; then
          ...
          printf "Is EOSIO installed in the default location: $HOME/eos/$EOSIO_VERSION (y/n)" && read -p " " PROCEED
        fi
      fi
      echo ""
      case $PROCEED in
  ...
  #export EOSIO_INSTALL_DIR="${EOSIO_DIR_PROMPT:-${HOME}/eos/${EOSIO_VERSION}}"
  export EOSIO_INSTALL_DIR="${EOSIO_DIR_PROMPT:-/usr/local}" 
}
...
function cdt-directory-prompt() {
  if [[ -z $CDT_DIR_PROMPT ]]; then
    ...
    while true; do
      if [[ $NONINTERACTIVE != true ]]; then
        printf "Is EOSIO.CDT installed in the default location? /usr/opt/eosio.cdt/1.6.2 (y/n)" && read -p " " PROCEED
      fi
      echo ""
      case $PROCEED in
        "" )
  ...
  export CDT_INSTALL_DIR="${CDT_DIR_PROMPT:-/usr/opt/eosio.cdt/1.6.2}"
}
