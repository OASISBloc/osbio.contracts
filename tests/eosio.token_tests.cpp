...
public:
   ...
   eosio_token_tester() {
      ...
      // Modified by wschoi
      create_accounts( { N(alice), N(bob), N(carol), N(osbio.token) } );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      ...
      // Modified by wschoi
      vector<char> data = get_row_by_account( N(osbio.token), acc, N(accounts), symbol_code );
      // Origin
   ...
   action_result create( account_name issuer,
      ...
      // Modified by wschoi
      return push_action( N(osbio.token), N(create), mvo()
           ( "issuer", issuer)
