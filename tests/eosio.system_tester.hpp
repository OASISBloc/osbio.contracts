...
public:
   ...
   void basic_setup() {
      ...
      create_accounts({ N(eosio.token), N(eosio.ram), N(eosio.ramfee), N(eosio.stake),
               N(osbio.bpay), N(eosio.vpay), N(osbio.saving), N(eosio.names), N(eosio.rex) });
      // Origin
      //create_accounts({ N(eosio.token), N(eosio.ram), N(eosio.ramfee), N(eosio.stake),
      ...
      // Modified by wschoi
      set_code( N(osbio.token), contracts::token_wasm());
...

      {
         ...
         // Modified  by wschoi
         const auto& accnt = control->db().get<account_object,by_name>( N(osbio.token) );
         // Origin
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      // Modified by wschoi
      create_currency( N(osbio.token), config::system_account_name, asset(100000000000000, core_symbol) );
      // Origin
   ...
   action_result reggen( const account_name& acnt, int params_fixture = 1 ) {
      action_result r = push_action( acnt, N(reggen), mvo()
                          ("producer",  acnt )
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      ...
      // Modified by wschoi
      vector<char> data = get_row_by_account( N(osbio.token), act, N(accounts), balance_symbol.to_symbol_code().value );
      // Origin
   ...
   void issue( const asset& amount, const name& manager = config::system_account_name ) {
      ...
      // Modified by wschoi
      base_tester::push_action( N(osbio.token), N(issue), manager, mutable_variant_object()
                                ("to",       manager )
                                ("quantity", amount )
                                ("memo",     "")
                                );
   ...
   void transfer( const name& from, const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      ...
      // Modified by wschoi
      base_tester::push_action( N(osbio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   void issue_and_transfer( const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      ...
      // Modified by wschoi
      trx.actions.emplace_back( get_action( N(osbio.token), N(issue),
      ...
      if ( to != manager ) {
         ...
         // Modified by wschoi
         trx.actions.emplace_back( get_action( N(osbio.token), N(transfer),
                                               vector<permission_level>{{manager, config::active_name}},
                                               mutable_variant_object()
                                               ("from",     manager)
                                               ("to",       to )
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      // Modified by wschoi
      vector<char> data = get_row_by_account( N(osbio.token), symbol_code, N(stat), symbol_code );
