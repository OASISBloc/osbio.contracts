...
BOOST_FIXTURE_TEST_CASE( producer_register_unregister, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice1111111), N(reggen), mvo()
...
BOOST_FIXTURE_TEST_CASE( proxy_actions_affect_producers, eosio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   // regproducer modified by wschoi
   BOOST_REQUIRE_EQUAL( success(), reggen( "defproducer1", 1) );
   BOOST_REQUIRE_EQUAL( success(), reggen( "defproducer2", 2) );
   BOOST_REQUIRE_EQUAL( success(), reggen( "defproducer3", 3) );
...
BOOST_FIXTURE_TEST_CASE(producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // regproducer modified by wschoi
   BOOST_REQUIRE_EQUAL(success(), reggen(N(defproducera)));
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      // Modified by wschoi
      const int64_t  initial_savings           = get_balance(N(osbio.saving)).get_amount();
      // Origin
      ...
      // Modifed by wschoi
      const int64_t  savings           = get_balance(N(osbio.saving)).get_amount();
      // Origin
   ...
   // test stability over a year
   {
      ...
      // regproducer modified by wschoi
      reggen(N(defproducerb));
      reggen(N(defproducerc));
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   std::vector<account_name> producer_names;
   {
      ...
      for (const auto& p: producer_names) {
         ...
         // regproducer modified by wschoi
         BOOST_REQUIRE_EQUAL( success(), reggen(p) );
...

   {
      ...
      // Modified by wschoi
      const asset    initial_bpay_balance      = get_balance(N(osbio.bpay));
      // Origin
      ...
      // Modified by wschoi
      const asset    bpay_balance      = get_balance(N(osbio.bpay));
      // Origin
...
BOOST_FIXTURE_TEST_CASE(votepay_share_invariant, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // regproducer modified by wschoi
   BOOST_REQUIRE_EQUAL( success(), reggen( proda ) );
   BOOST_REQUIRE_EQUAL( success(), reggen( prodb ) );
...
BOOST_FIXTURE_TEST_CASE(votepay_share_proxy, eosio_system_tester, * boost::unit_test::tolerance(1e-5)) try {
   ...
   // regproducer modified by wschoi
   BOOST_REQUIRE_EQUAL( success(), reggen( carol, 1) );
   BOOST_REQUIRE_EQUAL( success(), reggen( emily, 1) );
...
BOOST_FIXTURE_TEST_CASE(votepay_share_update_order, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // regproducer modified by wschoi
   BOOST_REQUIRE_EQUAL( success(), reggen( carol ) );
   BOOST_REQUIRE_EQUAL( success(), reggen( emily ) );
...
BOOST_FIXTURE_TEST_CASE(votepay_transition, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // regproducer modified by wschoi
   BOOST_REQUIRE_EQUAL( success(), reggen(N(defproducer1)) );
...
BOOST_AUTO_TEST_CASE(votepay_transition2, * boost::unit_test::tolerance(1e-10)) try {
   ...
   std::vector<account_name> producer_names;
   {
     ...
     t.setup_producer_accounts( producer_names, old_core_from_string("1.0000"),
      ...
      for (const auto& p: producer_names) {
         ...
         // regproducer modified by wschoi
         BOOST_REQUIRE_EQUAL( t.success(), t.reggen(p) );
   ...
   // regproducer modified by wschoi
   BOOST_REQUIRE_EQUAL( t.success(), t.reggen(producer_names[1]) );
...
BOOST_FIXTURE_TEST_CASE( vote_both_proxy_and_producers, eosio_system_tester ) try {
   ...
   // regproducer modified by wschoi
   BOOST_REQUIRE_EQUAL( success(), reggen( "carol1111111", 1) );
...
BOOST_FIXTURE_TEST_CASE( buyname, eosio_system_tester ) try {
   ...
   // regproducer modified by wschoi
   reggen( config::system_account_name );
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, eosio_system_tester ) try {
   ...
   // regproducer modified by wschoi
   BOOST_REQUIRE_EQUAL( success(), reggen( N(producer) ) );
...
BOOST_FIXTURE_TEST_CASE( eosioram_ramusage, eosio_system_tester ) try {
   ...
   // Modified by wschoi
   BOOST_REQUIRE_EQUAL( false, get_row_by_account( N(osbio.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );
   ...
   // Modified by wschoi
   base_tester::push_action( N(osbio.token), N(close), N(alice1111111), mvo()
                             ( "owner", "alice1111111" )
                             ( "symbol", symbol{CORE_SYM} )
   );
   BOOST_REQUIRE_EQUAL( true, get_row_by_account( N(osbio.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );
...
BOOST_AUTO_TEST_CASE( setabi_bios ) try {
   ...
   // Modified by wschoi
   t.create_account(N(osbio.token));
   t.set_abi( N(osbio.token), contracts::token_abi().data() );

   {
      // Origin
      //auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(eosio.token) );
      // Modified by wschoi
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(osbio.token) );
   ...
   // Modified by wschoi
   t.set_abi( N(osbio.token), contracts::system_abi().data() );
   // Origin
...
BOOST_FIXTURE_TEST_CASE( setabi, eosio_system_tester ) try {
   ...
   // Modified by wschoi
   set_abi( N(osbio.token), contracts::token_abi().data() );
   // Origin
   //set_abi( N(eosio.token), contracts::token_abi().data() );

   {
      // Modified by wschoi
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(osbio.token) );
      // Origin
