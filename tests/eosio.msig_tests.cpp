...
public:
  ...
  /* void issue( name to, const asset& amount, name manager = config::system_account_name ) {
   ...
   asset get_balance( const account_name& act ) {
      ...
      // Modified by wschoi
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(osbio.token), act, N(accounts)));
      // Origin
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, eosio_msig_tester ) try {
   ...
   set_authority(N(eosio), "active", authority(1,
      ...
      vector<permission_level_weight>{{{N(osbio.gens), config::active_name}, 1}}), "owner",
